...
f=open("googlenet75_india_pines.txt","w")
...
for j in range(0,Width):
    for i in range(0,Height):
        ...
        Classes[curr_tar].append(curr_inp)
        Classes_Index[curr_tar].append(curr_ind)
All_data = {}
...
def DataDivide(Classes_k,Num_Train_Each_Class_k,Classes_Index_k):
    ...
    np.random.seed(0)
    idx = np.random.choice(len(Classes_k), Num_Train_Each_Class_k, replace=False)#随机选择
    train_patch = [Classes_k[i] for i in idx]
    train_index = [Classes_Index_k[i] for i in idx]
    idx_test = np.setdiff1d(range(len(Classes_k)),idx)#求集合的差
...
Height1, Width1, Band1 = train_patch.shape[1], train_patch.shape[2], train_patch.shape[3]
Train_Label = []
for k in range(Num_Classes):
...
# Height1, Width1, Band1 = curr_patch.shape[0], curr_patch.shape[1], curr_patch.shape[2]
