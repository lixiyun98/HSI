...
import argparse
f=open("vggnet34_india_pines.txt","w")
...
conv3=con2d(h_conv2,W_conv3)+b_conv3
...
conv4=con2d(h_conv3,W_conv4)+b_conv4
...
conv5=con2d(h_conv4,W_conv5)+b_conv5
...
conv6=con2d(h_conv5,W_conv6)+b_conv6
conv6=batch_norm(conv6,True)
h_conv6=tf.nn.relu(conv6)

W_conv7=weight_variable([3,3,256,256])
b_conv7=bias_variable([256])
# h_conv7=tf.nn.relu(tf.layers.BatchNormalization(trainable=True)(con2d(h_conv6,W_conv7)+b_conv7))
# h_conv7=tf.nn.relu(con2d(h_conv6,W_conv7)+b_conv7)
conv7=con2d(h_conv6,W_conv7)+b_conv7
conv7=batch_norm(conv7,True)
h_conv7=tf.nn.relu(conv7)
...
conv8=con2d(h_conv7,W_conv8)+b_conv8
conv8=batch_norm(conv8,True)
h_conv8=tf.nn.relu(conv8)
...
conv9=con2d(h_conv8,W_conv9)+b_conv9
conv9=batch_norm(conv9,True)
h_conv9=tf.nn.relu(conv9)

W_conv10=weight_variable([3,3,512,512])
b_conv10=bias_variable([512])
# h_conv10=tf.nn.relu(tf.layers.BatchNormalization(trainable=True)(con2d(h_conv9,W_conv10)+b_conv10))
# h_conv10=tf.nn.relu(con2d(h_conv9,W_conv10)+b_conv10)
conv10=con2d(h_conv9,W_conv10)+b_conv10
conv10=batch_norm(conv10,True)
h_conv10=tf.nn.relu(conv10)

W_conv11=weight_variable([3,3,512,512])
b_conv11=bias_variable([512])
# h_conv11=tf.nn.relu(tf.layers.BatchNormalization(trainable=True)(con2d(h_conv10,W_conv11)+b_conv11))
# h_conv11=tf.nn.relu(con2d(h_conv10,W_conv11)+b_conv11)
conv11=con2d(h_conv10,W_conv11)+b_conv11
conv11=batch_norm(conv11,True)
h_conv11=tf.nn.relu(conv11)

W_conv12=weight_variable([3,3,512,64])
b_conv12=bias_variable([64])
# h_conv12=tf.nn.relu(tf.layers.BatchNormalization(trainable=True)(con2d(h_conv11,W_conv12)+b_conv12))
# h_conv12=tf.nn.relu(con2d(h_conv11,W_conv12)+b_conv12)
conv12=con2d(h_conv11,W_conv12)+b_conv12
conv12=batch_norm(conv12,True)
h_conv12=tf.nn.relu(conv12)

W_fc1=weight_variable([size*size*64,128])
b_fc1=bias_variable([128])
h_flat=tf.reshape(h_conv12,[-1,size*size*64])
